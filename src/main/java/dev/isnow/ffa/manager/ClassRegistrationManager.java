...
public class ClassRegistrationManager {
	...
	public void loadListeners(String packageName) {
		for (Class<?> clazz : getClassesInPackage(packageName)) {
			if (isListener(clazz)) {
				try {
					Bukkit.getPluginManager().registerEvents((Listener) clazz.newInstance(), TaklyFFA.INSTANCE.getPlugin());
	...
	public Collection<Class<?>> getClassesInPackage(String packageName) {
		...
		CodeSource codeSource = TaklyFFA.INSTANCE.getPlugin().getClass().getProtectionDomain().getCodeSource();
