...
public class CommandFramework implements CommandExecutor {
	...
	private final TaklyFFA plugin;
	private Map<String, Entry<Method, Object>> commandMap = new HashMap<>();
	private CommandMap map;

	public CommandFramework(TaklyFFA plugin) {
		this.plugin = plugin;
	...
	public void registerCommand(Command command, String label, Method m, Object obj) {
		...
		commandMap.put(this.plugin.getPlugin().getName() + ':' + label.toLowerCase(), new AbstractMap.SimpleEntry<>(m, obj));

		String cmdLabel = label.replace(".", ",").split(",")[0].toLowerCase();
		if (map.getCommand(cmdLabel) == null) {
			org.bukkit.command.Command cmd = new BukkitCommand(cmdLabel, this, plugin.getPlugin());
			map.register(plugin.getPlugin().getName(), cmd);
		}
	...
	public void registerCompleter(String label, Method m, Object obj) {
		String cmdLabel = label.replace(".", ",").split(",")[0].toLowerCase();
		if (map.getCommand(cmdLabel) == null) {
			org.bukkit.command.Command command = new BukkitCommand(cmdLabel, this, plugin.getPlugin());
			map.register(plugin.getPlugin().getName(), command);
		}
